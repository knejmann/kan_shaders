uniform sampler2D adsk_results_pass2, adsk_results_pass3, adsk_results_pass4;
...
void main() {
	...
	float cr_str = texture2D(adsk_results_pass2, uv).r;
	...
	if(add_distortion) {
		...
		vec4 sumw = vec4(0.0);
		float dist_str = texture2D(adsk_results_pass2, uv).r;
